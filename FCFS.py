...
def FCFS(process_list, context_switching):
    ...
    process_sorted_list = sorted(
        process_list, key=lambda Process: Process.arrival_time)
    ...
    current_time = 0
    # min heap to store current processes and return process with smallest arrival time
    q = Q.PriorityQueue()
    ...
    while len(process_sorted_list) > 0:
        ...
        while (len(process_sorted_list) > 0) and(process_sorted_list[0].arrival_time <= current_time):
            q.put(((process_sorted_list[0].arrival_time,
                    process_sorted_list[0].idd), process_sorted_list[0]))
            process_sorted_list.pop(0)

        while not q.empty():
            arrival, process = q.get()
            ...
            while (len(process_sorted_list) > 0) and(process_sorted_list[0].arrival_time <= current_time):
                q.put(
                    ((process_sorted_list[0].arrival_time, process_sorted_list[0].idd), process_sorted_list[0]))
                process_sorted_list.pop(0)
    ...
    for i in range(0, len(result)):
        ...
        running_time[(result[i].start_time, result[i].finish_time)
                     ] = result[i].idd
    ...
    for i in range(len(result)):
        ...
        metric[i, 2] = result[i].finish_time - \
            result[i].arrival_time  # TAT = finish - arrival
        # Wating = TAT - burst_time
        metric[i, 1] = metric[i, 2]-result[i].burst_time
        metric[i, 3] = 1.*metric[i, 2] / \
            result[i].burst_time  # WTAT = TAT/burst_time
